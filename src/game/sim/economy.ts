...
/** 보유 연구 효과 합산. */
export const researchMods = (s: GameState): ResearchMods => {
  const m: ResearchMods = {
    featureCostMult: 1,
    usersGainMult: 1,
    autoLoc: 0,
    bugMult: 1,
  ...
  for (const id of s.research) {
    ...
    if (!r) continue;
    if (r.featureCostMult) m.featureCostMult *= r.featureCostMult;
    if (r.usersGainMult) m.usersGainMult *= r.usersGainMult;
    if (r.autoLoc) m.autoLoc += r.autoLoc;
    if (r.bugMult) m.bugMult *= r.bugMult;
    if (r.debtDecay) m.debtDecay += r.debtDecay;
  }
  return m;
};

/** 도전과제 전역 배율: 1 + 1% * 달성 수 */
export const achievementMult = (s: GameState): number => 1 + 0.01 * s.achievements.length;
...
/** 부채 생산성 페널티 배율: 1/(1 + debt/softcap), 하한 PROD_FLOOR로 클램프 */
export const prodMult = (s: GameState): number =>
  Math.max(B.PROD_FLOOR, 1 / (1 + s.debt / B.DEBT_SOFTCAP));

/** 부채/페널티 적용 전 raw LoC/초 (개발자 합산) */
export const rawLoc = (s: GameState): number =>
  DEVS.reduce((sum, d) => sum + s.devs[d.id] * d.loc, 0);

/** 실제 LoC/초 (부채·경력·업그레이드·연구·도전과제 반영) */
export const locPerSec = (s: GameState): number =>
  (rawLoc(s) * prodMult(s) * careerMult(s) * upgradeMods(s).prod + researchMods(s).autoLoc) *
  achievementMult(s);
...
/** ₩/초 (유저 × 단가, 부채·경력·업그레이드·도전과제 반영) */
export const wonPerSec = (s: GameState): number =>
  s.users * B.REV_PER_USER * prodMult(s) * careerMult(s) * upgradeMods(s).prod * achievementMult(s);

/** 코드 짜기 클릭 1회 LoC 획득량 (업그레이드·도전과제 반영) */
export const clickPow = (s: GameState): number =>
  (1 + s.career) * careerMult(s) * upgradeMods(s).click * achievementMult(s);
...
/** 다음 기능 출시 LoC 비용 (연구 할인 반영) */
export const featureCost = (s: GameState): number =>
  Math.floor(B.FEATURE_BASE * Math.pow(B.FEATURE_GROW, s.features) * researchMods(s).featureCostMult);

/** 기능 출시 시 유저 증가량 (연구 배율 반영) */
export const usersGain = (s: GameState): number =>
  Math.ceil((1 + s.features * B.USERS_PER_FEATURE) * researchMods(s).usersGainMult);
